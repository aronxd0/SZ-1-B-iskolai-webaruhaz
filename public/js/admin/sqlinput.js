function SQLinput(pushHistory = true) {
    $("#content_hely").fadeOut(300, function() {
        ...
        if (pushHistory) {
            ...
            history.pushState({ view: 'sql' }, 'sql', '#sql');
        }
    });
}
    ...
    try{
        ...
        if (adat.select == true) {
            if (adat.adat.rows.length > 0) {
                ...
                // elso sor betölt 
                for (var nevek of tablamevek) { html +=  `<th class="p-2 ">${ nevek.toString()}</th>`; }
                html += "</tr></thead><tbody>"
                for (var item of adat.adat.rows) {
                    ...
                    for (var nev of tablamevek) { html += `<td class='p-2'>${item[nev]}</td>`; }
                    html += "</tr>";
                }
                html += "</tbody></table>";
        ...
        var figyelmeztetes = ` 
            <div role="alert" class="bg-red-200 text-red-700 dark:bg-red-950/50 dark:text-red-400 px-4 py-3 rounded-4" style="border: none;">
